...
-(NSString *)encodingName
{
    ...
    if(mac)
    {
        ...
        if(!macalternatives) macalternatives=@{XADASCIIStringEncodingName.lowercaseString: XADMacOSRomanStringEncodingName,
            XADWindowsCP1252StringEncodingName.lowercaseString: XADMacOSRomanStringEncodingName,
            XADShiftJISStringEncodingName.lowercaseString: XADMacOSJapaneseStringEncodingName};
...
-(void)setFixedEncoding:(NSStringEncoding)encoding
{
    ...
    else self.fixedEncodingName = [XADString encodingNameForEncoding:encoding];
}
